...
@section('page-actions')
    <div class="btn-group">
        ...
        <a href="{{ route('admin.payment-methods.show', $paymentMethod) }}" class="btn btn-info">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                @if($paymentMethod->icon)
                                    <div id="current-icon-preview" class="mt-2">
                                        <strong class="text-white">Current:</strong> {!! $paymentMethod->icon_html !!}
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                                       id="color" name="color" value="{{ old('color', $paymentMethod->color) }}">
                                @error('color')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('processing_fee_fixed') is-invalid @enderror" 
                                       ...
                                       value="{{ old('processing_fee_fixed', $paymentMethod->processing_fee_fixed) }}" 
                                       step="0.01" min="0">
                    ...
                    @if($paymentMethod->processing_fee_percentage > 0 || $paymentMethod->processing_fee_fixed > 0)
                        <div class="alert alert-info">
                            <strong>Current Fee Structure:</strong> {{ $paymentMethod->formatted_processing_fee }}
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    ...
                    @if($paymentMethod->minimum_amount || $paymentMethod->maximum_amount)
                        <div class="alert alert-info">
                            <strong>Current Limits:</strong> {{ $paymentMethod->amount_limits_text }}
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label text-white" for="is_active">
                                ...
                                <br><small class="text-muted">Allow customers to use this payment method</small>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 text-white">
                        ...
                        <div class="mt-2">
                            ...
                            <div class="d-flex justify-content-between mt-1">
                                ...
                                <span class="badge bg-secondary">{{ $paymentMethod->sort_order }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('instructions') is-invalid @enderror" 
                                  ...
                                  placeholder="Instructions shown to customers when they select this payment method">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                        @error('instructions')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('api_key') is-invalid @enderror" 
                               id="api_key" name="api_key" value="{{ old('api_key', $paymentMethod->api_key) }}">
                        @error('api_key')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="password" class="form-control @error('api_secret') is-invalid @enderror" 
                               id="api_secret" name="api_secret" value="{{ old('api_secret', $paymentMethod->api_secret) }}"
                               placeholder="{{ $paymentMethod->api_secret ? '••••••••••••' : '' }}">
                        @error('api_secret')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        @if($paymentMethod->api_secret)
                            <div class="form-text text-white">Leave empty to keep current secret</div>
                        @endif
                    </div>
...
$(document).ready(function() {
    ...
    function updateIconPreview() {
        ...
        if (icon) {
            ...
            $('#icon-preview').html('<strong>New Preview:</strong> ' + preview);
        } else {
            $('#icon-preview').remove();
        }
    }
